...
- `npm run docker -- compose up --build -d` foi validado localmente com MySQL, backend e frontend.
...
- Cobertura atual valida vivacidade, prontidão, listagem, criação, alias legado, erros 400 e limite de requisições.
- Teste de fumaça real com MySQL em Docker foi validado via API e proxy Nginx.
- Ambiente de desenvolvimento local foi validado com backend e frontend no host usando MySQL em container.
...
- `npm run docker -- compose up --build -d`: passou com compilação real de MySQL, backend e frontend.
- `npm run docker -- compose ps`: passou com containers saudáveis.
- Teste de fumaça Docker via `http://127.0.0.1:4200/api/v1/tasks`: passou.
- Teste de fumaça local dev via `http://127.0.0.1:4200/api/v1/tasks`: passou com backend/frontend no host e MySQL em container.
