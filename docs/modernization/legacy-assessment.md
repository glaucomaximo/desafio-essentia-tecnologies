...
- Frontend em Angular 22.1.5 standalone, zoneless, Signals, Reactive Forms e Tailwind CSS 4.3.3.
...
- Frontend: Angular 22.1.5, TypeScript 6.0.3, RxJS 7.8.x e Tailwind CSS 4.3.3.
- TypeScript 7.0.2 existe no npm, mas permanece bloqueado porque Angular 22.1.5 declara suporte oficial a TypeScript `>=6.0 <6.1`.
...
- `frontend/src/app/core`: clientes HTTP, armazenamento de sessão SSR-safe e fachada reativa.
- `frontend/src/app/features/auth`: componente standalone de autenticação.
- `frontend/src/app/features/tasks`: componentes standalone de formulário e lista de tarefas.
...
- Sem testes automatizados de frontend nesta rodada; compilação Angular de desenvolvimento valida templates, tipagem estrita e chunk lazy gerado por `@defer`.
...
| LEG-018 | MÉDIA       | Frontend, Performance            | Angular                 | Frontend ainda dependia de `zone.js`, diretivas estruturais legadas e lógica concentrada no componente raiz.                                       | Maior acoplamento, menor previsibilidade em SSR/hydration e pior aderência ao Angular moderno. | Média         | Migrar para aplicação zoneless, Signals, control flow novo, componentes standalone e `@defer`.                               | Médio       | Médio              | Mitigado em `2.1.0`              |
| LEG-019 | INFORMATIVA | Dependências                     | TypeScript              | TypeScript 7.0.2 está publicado, mas Angular 22.1.5 exige TypeScript `>=6.0 <6.1`.                                                                 | Upgrade imediato deixaria o projeto fora do suporte oficial do Angular.                        | Alta          | Manter TypeScript 6.0.3 até o Angular publicar peer dependency compatível com TypeScript 7.                                  | Baixo       | Médio              | Decisão bloqueada                |
...
- `npm --workspace frontend run typecheck`: passou após migração zoneless, Tailwind 4.3.3 e control flow novo; build gerou chunk lazy `task-list-component`.
...
- P2 concluído: modernizar frontend para Angular zoneless, Signals, control flow novo e Tailwind CSS 4.3.3.
